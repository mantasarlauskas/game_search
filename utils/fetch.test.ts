...
describe('fetch', () => {
    describe('getFetchUrl', () => {
        it('returns url', () => {
            ...
            expect(
                getFetchUrl('url', { ordering: SortMode.NAME, genres: '123' })
            ).toContain('/api/url?ordering=name&genres=123');
        });
    });
    ...
    describe('getQueryParams', () => {
        it('returns query params', () => {
            ...
            expect(getQueryParams({})).toEqual('');
            expect(getQueryParams({ ordering: SortMode.NAME })).toEqual(
                'ordering=name'
            );
            expect(
                getQueryParams({ ordering: SortMode.NAME, genres: '123' })
            ).toEqual('ordering=name&genres=123');
        });
    });
    ...
    describe('fetchData', () => {
        it('returns data', async () => {
            window.fetch = jest
                .fn()
                .mockResolvedValue({ json: getGames, ok: true });
            const res = await fetchData('url', { platforms: '123' });
            expect(res).toEqual(getGames());
            expect(window.fetch).toBeCalledTimes(1);
            expect(window.fetch).toBeCalledWith(
                expect.stringContaining('/api/url?platforms=123')
            );
        });
    });
});
